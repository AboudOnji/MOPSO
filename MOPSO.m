...
% Select and display the best solution found
best_solution = selectBestSolution(REP);
disp('Best Solution Found:');
disp(['Position: ', num2str(best_solution.pos)]);
disp(['Fitness: ', num2str(best_solution.pos_fit)]);
...
% Function that selects the best solution based on an aggregation function
function best_solution = selectBestSolution(REP)
    % Assuming a simple aggregation function: sum of the objectives
    % (minimization problem, hence sum of objectives should be minimal)
    [~, idx] = min(sum(REP.pos_fit, 2));
    best_solution.pos = REP.pos(idx, :);
    best_solution.pos_fit = REP.pos_fit(idx, :);
end
